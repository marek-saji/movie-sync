...
const request = createRequest(TRAKT_API_BASE, {
    ...
    processResponse: async (response, pathname, requestOptions) => {
        ...
        )
        {
            ...
            printDebug('Hit Trakt request rate limit', rateLimit.name, '; Will retry in', retryAfterSec, 'seconds');
        ...
        const queueCheck = () => {
            setTimeout(async () => {
                ...
                else
                {
                    ...
                    if (response.ok)
                    {
                        printMsg('Authenticated in Trakt');
...
{
    printLog('Refreshing Trakt access token');
    ...
    if (!response.ok)
    {
        ...
        printWarn('Failed to refresh Trakt token:', errMsg);
        return null;
    }
...
async function traktGetAuthToken ()
{
    ...
    if (hasToken && hasExpiresAt)
    {
        ...
        if (expiresAt <= new Date())
        {
            ...
            {
                printMsg('Stored Trakt token expired at', formatTime(expiresAt));
            }
        }
    }
    ...
    if (auth)
    {
        ...
        printMsg('Saving Trakt authentication');
...
{
    printMsg('Saving new entries to Trakt');
    ...
    const viewLogNew = viewLog
        .filter(
            ({ time, movie }) => {
                const matches = traktViewLog
                    .filter(
                        (h) => Math.abs(h.time - time)
                    < VIEW_LOG_CLOSE_ENOUGH_MS,
                    )
                    .filter((h) => movieMatches(h.movie, movie));
                printDebug(
                    `“${movie.title}” (${movie.year})`,
                    movie.services[0].url,
                    matches.length === 0
                        ? 'not found in Trakt view log — will add'
                        : `alraedy on Trakt view log ${matches.map((match) => match.movie.services[0].url).join(' ')}`,
                );
                return matches.length === 0;
            },
        );

    printMsg('Entries missing on Trakt:', viewLogNew.length);
    ...
    if (misses.length)
    {
        printLog('Looking up entries that failed to sync to Trakt easily (movie name mismatch):', misses.length);

        const viewLogRoundTwo = [];
        (await Promise.all(
            misses.map(async (viewLogEntry) => {
                const { time, movie } = viewLogEntry;
                printDebug('Looking up movie on Trakt:', movie);
                const foundMovie =
        ...
        {
            printLog('Syncing looked up movies to Trakt:', viewLogRoundTwo.length);
            ...
            assert.strictEqual(
                ...
                0,
                'Failed to sync view log of entries looked up on Trakt',
            );
        }
    }
    ...
    {
        printErr('Failed to look up', lookupMisses.length, 'movies on Trakt.');
    ...
    {
        printMsg('Saved', viewLogNew.length, 'new entries on Trakt. Skipped', viewLog.length - viewLogNew.length, 'already existing ones.');
    }
