...
const request = createRequest(TRAKT_API_BASE, {
    ...
    processResponse: async (response, pathname, requestOptions) => {
        ...
        )
        {
            ...
            printDebug('Hit request rate limit', rateLimit.name, '; Will retry in', retryAfterSec, 'seconds');
...
{
    if (!config['trakt-access-token'])
    {
        const auth = await traktAuth();
        const configUpdate = {
            'trakt-access-token': auth.access_token,
            'trakt-expires-at': auth.created_at + auth.expires_in,
...
async function traktFetchViewLog (start, end)
{
    printLog(
        ...
        'from', timeFormatter.format(start),
...
export async function traktSyncViewLog (viewLog)
{
    ...
    if (misses.length)
    {
        printLog('Looking up entries that failed to sync easily (movie name mismatch):', misses.length);

        const viewLogRoundTwo = [];
        (await Promise.all(
            misses.map(async (viewLogEntry) => {
                const { time, movie } = viewLogEntry;
                printDebug('Looking up movie:', movie);
                const foundMovie =
        ...
        {
            printLog('Syncing looked up movies:', viewLogRoundTwo.length);
    ...
    if (lookupMisses.length)
    {
        ...
        lookupMisses.forEach(({ time, movie }) => {
            ...
            printLog('  watched at ', timeFormatter.format(time));
