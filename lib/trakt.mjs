 ...
 * @return {object}
 */
export async function traktAuth ()
{
...
async function traktSyncViewLogRequest (accessToken, viewLog)
{
    if (viewLog.length === 0)
    {
        return [];
 ...
 * @return {ViewLog}
 */
async function traktFetchHistory (accessToken, start, end)
{
 ...
 * @return {void}
 */
export async function traktSyncViewLog (accessToken, viewLog)
{
    const times = viewLog.map(({ time }) => time).sort((a, b) => a - b);
    const minTime = times.at(0);
    const maxTime = times.at(-1);

    const history =
        await traktFetchHistory(accessToken, minTime, maxTime);
    ...
    const misses =
        await traktSyncViewLogRequest(accessToken, viewLogNew);
    ...
    const roundTwoMisses =
      await traktSyncViewLogRequest(accessToken, viewLogRoundTwo);
